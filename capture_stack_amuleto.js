...
const OUT_DIR = '/Users/eduardorolim/Documents/Documentos Atual - MacBook Air de Eduardo/carrossel-v2-edu-99hud/output/stack-amuleto';
const PORT = 8882;

function startServer() {
  const server = http.createServer((req, res) => {
    let filePath = path.join(OUT_DIR, req.url === '/' ? 'carrossel.html' : req.url);
...
(async () => {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const bgColor = (classes.includes('slide-cta') || classes.includes('slide-mini-cta') || classes.includes('slide-tipo-c'))
      ? '#ffffff' : classes.includes('slide-tipo-a') ? '#0d0d0d' : '#000000';
